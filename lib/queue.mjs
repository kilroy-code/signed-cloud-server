...
['Team', 'KeyRecovery', 'EncryptionKey'].forEach(collectionName => 
  queue = queue.then(() => fs.mkdir(tagPath(collectionName, ''), {recursive: true})));

export async function enqueueRequest(req, res, next) { // Middleware to adds the req to a common FIFO queue.
